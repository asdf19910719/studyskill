...
- 根据已有笔记初始化学习系统。
- 初始化时抽取关键概念，生成 `知识地图.md`，按概念判断资料深度、熟悉度、先修关系和下一步动作。
- 判断资料是否足够支撑系统学习。
- 在工具允许时检索官方文档和高质量资料，扩展薄弱笔记。
- 对陌生先修概念先做“概念预热”，不直接跳到系统笔记或大任务。
...
初始化阶段现在不是只给整份资料打分，而是按这个顺序执行：

```text
扫描学习材料
→ 生成 学习材料索引.md
→ 生成 知识地图.md
→ 写入 learning_state.json.concepts
→ 判断当前应该 概念预热 / 扩展笔记 / 继续学习
```
...
当前熟悉度采用保守策略：脚本默认标记为 `陌生`，后续应通过用户自评、最小自测和考试结果逐步校正。资料深度和下一步动作由材料内容、内置概念规则和先修关系推导。

学习方法已从“五问学习法”升级为“六问学习法”：
...
```bash
mkdir -p ~/.codex/skills/universal-learning-coach
cp -r universal-learning-coach/* ~/.codex/skills/universal-learning-coach/
```

项目级安装：

```text
<ObsidianVault>/.codex/skills/universal-learning-coach/SKILL.md
```
